...
def find_temperature(
    ...
    distance_type: Optional[str]
        Distance type to be used for retrieval. Either has to be "inner_product", "cosine" or "l2".
    alpha: float
        Pre-defined confidence level for the experiments.
    num_attempts: int
        Number of attempts to be used during search.
    search_space: Tuple[float, float]
        Define the interval in which the temperature should be searched for.
    num_batches: int
        Number of batches used for search.
    num_neighbors: int
    ...
    datastore_dir: str
        Path to the directory where the datastore should be saved.
    ignore_token_ids: Tuple[int, ...]
        Tuple of tokens to ignore during the temperature search.
