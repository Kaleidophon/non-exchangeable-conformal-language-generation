...
def evaluate_generations(
    ...
    for batch in tqdm(data_loader, total=len(data_loader)):
        for n in range(num_samples):
            ...
            if n == 0:
                all_inputs += tokenizer.batch_decode(batch["input_ids"], skip_special_tokens=True, clean_up_tokenization_spaces=True)
    ...
    if task == "mt":
        ...
        partial_results = [
            evaluate_translation_model(
                generations[n], source_path, reference_path, metrics=evaluation_metrics
            )
            for n in range(num_samples)
        ]
    ...
    generations_path = f"{result_dir}/{timestamp}_{model_identifier.replace('/', '_')}_{generation_method}_generations.csv"
    generations_df = pd.DataFrame(columns=["source", "reference", "generation"])

    if task == "mt":
        with codecs.open(source_path, "r", "utf-8") as source_file:
            source_lines = source_file.readlines()

    with codecs.open(reference_path, "r", "utf-8") as reference_file:
        reference_lines = reference_file.readlines()

    for i, samples in enumerate(all_outputs):

        if task == "mt":
            generations_df.at[i, "source"] = source_lines[i]

        elif task == "lm":
            generations_df.at[i, "source"] = all_inputs[i]

        generations_df.at[i, "reference"] = reference_lines[i]
        generations_df.at[i, "generation"] = all_outputs[i]

    generations_df.to_csv(generations_path, sep="\t")
